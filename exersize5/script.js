...
for (let imageCount = 0; imageCount < 30; imageCount = imageCount + 1) {
    ...
    setTimeout(function(){
        ...
        const image = addImage(currentImage, Math.random() * 360);

        image.style.transition = 'filter 30s';
        setTimeout(function(){
    
            image.style.filter = `grayscale(1) sepia(1) hue-rotate(180deg)`;
